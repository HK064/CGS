...
public class PlayPanel extends CGSPanel {
     ...
     */
    protected Card drawCards(Graphics g, int x, int y, int w, int h, List<Card> cards, List<Card> selectedCards,
            int type) {
        ...
        for (int i = 0; i < cards.size(); i++) {
            ...
            // マウスオーバー？
            if ((x + cardXPos + cardGap * i <= mousePos.x) && (mousePos.x < x + cardXPos + cardGap * i + cardWidth)
                    && (y + cardYPos - selected * cardPopGap <= mousePos.y) && (mousePos.y < y + cardYPos + cardHeight)
                    && (type == 1)) {
        ...
        for (int i = 0; i < cards.size(); i++) {
            ...
            drawCard(g, x + cardXPos + cardGap * i, y + cardYPos - selected * cardPopGap, cardWidth, cardHeight,
                    cards.get(i));
        }
        return selectedCard;
    }
